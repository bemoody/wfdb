...
/* Output .mat data storage types (values of mattype), defined by the .mat format 
These variables are for the sub4 tag data STORAGE type */  
...
char *argv[];
{
    ...
    /* The entire file is composed of: 
       ...
       - 8 byte master tag. 4 bytes indicate data type = matrix, 4 bytes indicate data size. 
       - 4 subelements. Each subelement has a 4 byte tag giving the data type of the elements,
         a 4 byte tag giving the subelement size, and the subelement's actual content.
    ...
    */
    static char prolog[192];  /* 128 byte descriptive text + 64 bytes of fixed length content */
    ...
    int highres = 0, i, isiglist, mattype, nisig, nosig = 0, pflag = 0,
      mastertype = 14, sub1type = 6, sub2type = 5, sub3type = 1, sub4type, fieldversion = 256,
      s, sfname = 0, *sig = NULL, stat = 0, vflag = 0, sub1class = 6,
      nbytesperelement, wfdbtype;
    
    /* nbytesmaster is the value specified in the master tag representing the size
       of all following content. 
       nbytesofdata gives the number of bytes of signal data 
       lremain stores the number of bytes to pad to make the file size a multiple of 8
    ...
    */    
    ...
    /* Allocate space for output signals: vo so sig */
    ...
    *p = 'm';	/* append 'm' to the output record name */
	...
	switch (si[sig[i]].fmt) {	
	case 24:
	case 32: sub4type = MAT32; wfdbtype = 32; nbytesperelement=4; break;
	case 80: break;
	default: if (sub4type != MAT32) sub4type = MAT16; wfdbtype = 16; nbytesperelement=2; break;
	}
    ...
    lremain = nbytesofdata%8; /* This is the remaining no. bytes that don't fit into integer
			       multiple of 8. ie if 18 bytes, lremain=2, from 17 to 18. */
    
    /* nbytesmaster= (8 + 8) + (8 + 8) + (8 + 8) + (8 + nbytesofdata) + padding. Must be integer multiple 8. */
    if (lremain==0){
    ...
    prolog[124] = fieldversion & 0xff; /* Bytes 125-126 indicate version, set to 0x0100 hex = 256.*/
    prolog[125] = (fieldversion >> 8) & 0xff;
    sprintf(prolog + 126, "I");   /* Characters IM to indicate little endian */
    sprintf(prolog + 127, "M");

    /* 8 byte MASTER TAG, followed by the actual data. First 4 is data type, next 4 is number of bytes of entire field. */
    prolog[128] = mastertype & 0xff; /* Data type is always 14 for matrix. */
    /* Number of bytes of data element=(8 + 8) + (8 + 8) + (8 + 8) + (8 + Nvalues*bytespervalue) = 56 + Nvalues*bytespervalue */
    prolog[132] = nbytesmaster & 0xff;
    prolog[133] = (nbytesmaster >> 8) & 0xff;
    prolog[134] = (nbytesmaster >> 16) & 0xff;
    prolog[135] = (nbytesmaster >> 24) & 0xff;

    /* Matrix data has 4 subelements (5 if imag): Array flags, dimensions array, array name, real part. Each subelement has its own subtag, and subdata. */
   
    /* Subelement 1: Array flags. */
    prolog[136] = (sub1type & 0xff); /* Sub tag 1: type */
    prolog[140] = 8 & 0xff; /* Sub tag 1: size */
    prolog[144] = sub1class & 0xff; /*Value class, indicating the MATLAB data type. NOT the same as sub4type. */
    ...
    /* Total size of everything before actual data = 128 byte header + 8 byte master tag + 56 byte Subelements (48 byte default + 8 byte name) = 192. */
    wfdbputprolog((char *)prolog, 192, 0);  /* Write the specified prolog to the output signal */
