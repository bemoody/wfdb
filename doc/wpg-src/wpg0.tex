...
@noindent
The @code{sample} function allows the caller to read samples in any
order.  The first argument is a signal number (a non-negative integer),
and the second is a time, expressed as a non-negative sample number.
If @code{sample} is invoked with valid input arguments, the companion
function @code{sample_valid} returns 1.
...
@code{sample} attempts to return a reasonable value.  If @var{t} is
