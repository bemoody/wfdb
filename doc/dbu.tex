...
@sp 4
@center Tenth Edition (revised and with additions for WFDB library version 10.0.1)
@center @today
@sp 5
...
Copyright @copyright{} 1989 -- 1999 George B. Moody
@sp 2
...
the @cite{WFDB Programmer's Guide} (May, 1999), with
additional material for release 10.0.0 of the WFDB library.
@end ifinfo
...
not wish to allow access to remote files, or if libwww is not available for
your OS, simply do not define the symbol NETFILES when compiling the WFDB
...
@example
bcc -L@var{LIBDIR} psamples.c wfdb.lib
@end example

@noindent
where @var{LIBDIR} is the directory in which @samp{wfdb.lib} (the WFDB
...
@comment  node-name,  next,  previous,  up
@unnumberedsubsec Using the WFDB library with other languages
@cindex C++ bindings
...
first of these directories named in the WFDB path.  Any other matching
...
of the WFDB library.  These names are used for functions, global variables
...
compiled with the @samp{-lwfdb} option.  The functions are introduced in
...
@code{WFDB_Anninfo} structures in @var{aiarray}; @var{nann} must be no
greater than twice @code{WFDB_MAXANN} (a constant defined in
@file{<wfdb/wfdb.h>}), and no more than @code{WFDB_MAXANN} input and
@code{WFDB_MAXANN} output annotators may be open at once.  The caller must
...
@code{WFDB_Siginfo} structures in @var{siarray}, @var{nsig}, need never be
greater than @code{WFDB_MAXSIG} (a constant defined in @file{<wfdb/wfdb.h>});
...
@var{nsig}.  The caller must allocate storage for the @code{WFDB_Siginfo}
...
The array of @code{WFDB_Siginfo} structures is declared @code{static}, a
...
@code{WFDB_MAXSIG}.  (On existing CD-ROMs containing database records, there is
...
@code{WFDB_Siginfo} structures in @var{siarray}; @var{nsig} must be no
greater than @code{WFDB_MAXSIG} (a constant defined in @file{<wfdb/wfdb.h>}).
...
@code{isigopen} or @code{wfdbinit}; this number will never exceed
@code{WFDB_MAXSIG}, defined in @file{<wfdb/wfdb.h>}.)  On return,
...
how many input signals there are.  The array of @code{WFDB_Siginfo}
...
@code{osigopen}; this number must never exceed @code{WFDB_MAXSIG}, defined in
@file{<wfdb/wfdb.h>}.)  On entry, @var{vector[i]} contains the next sample
...
files, but before calling @code{dbquit}.wfdbIf @var{record} begins with
...
nsig = isigopen("old", s, -WFDB_MAXSIG);
if (nsig > 0) @{
    s[0].gain = 100.0;
...
@noindent
Unless the application has used @code{setgvmode(WFDB_HIGHRES)} and has
...
@noindent
This function may be used to set or change the database path (@pxref{WFDB
...
@noindent
This function attempts to locate an existing WFDB file by searching the database
path (@pxref{WFDB path}).  Normally, the file is specified by its @var{type}
...
as part of its name can be found by @code{wfdbfile} if the name is passed in the
@var{type} variable and @var{record} is @code{NULL}.  The string returned by
@code{wfdbfile} includes the appropriate component of the database path;  since
...
@code{wfdbfile} returns @code{NULL}.  (This function was first introduced
in WFDB library version 4.3.)
...
int nsig;
WFDB_Signal s;
static WFDB_Siginfo siarray[WFDB_MAXSIG];

@dots{}

if ((nsig = isigopen(record, siarray, WFDB_MAXSIG)) < 1)
    exit(1);
for (s = 0; s < nsig; s++) @{
...
Until WFDB library version 8.2, these types were known as @code{struct
...
@file{<wfdb/wfdb.h>}, but their use is discouraged.
...
2 and @code{WFDB_MAXSPF} (defined in @file{<wfdb/wfdb.h>}).  Note that
...
their first arguments.  There may be no more than @code{WFDB_MAXSIG}
(defined in @file{<wfdb/wfdb.h>}) input signals and @code{WFDB_MAXSIG} output
...
of which are commonly used as parts of WFDB file names) as annotator
...
path (@pxref{WFDB path}).  Under UNIX, it is possible to create or find
...
no more than @code{WFDB_MAXANN} (defined in @file{<wfdb/wfdb.h>}) input
annotators and @code{WFDB_MAXANN} output annotators open at once.
@iftex
...
@file{<wfdb/ecgcodes.h>}). @xref{Annotation Codes}, for a list of
...
@file{wfdbcal}, in the @file{udb} directory.
...
@file{16l} (``one-six-ell'') specifies up to @code{WFDB_MAXSIG} format 16
files, and record @file{8l} (``eight-ell'') specifies up to
@code{WFDB_MAXSIG} format 8 files, named @file{data0}, @file{data1},
...
@example
 ...
 @i{6}  char *argv[];
 @i{7}  @{
 @i{8}      static WFDB_Siginfo s[WFDB_MAXSIG];
 @i{9}      int i, nsig;
@i{10}
@i{11}      if (argc < 2) @{
@i{12}          fprintf(stderr, "usage: %s record\n", argv[0]);
@i{13}          exit(1);
@i{14}      @}
@i{15}      nsig = isigopen(argv[1], s, WFDB_MAXSIG);
...
@i{25}          printf(" Gain: ");
@i{26}          if (s[i].gain == 0.)
@i{27}              printf("uncalibrated; assume %g", WFDB_DEFGAIN);
...
@table @emph
@item Line 8:
There might be as many as @code{WFDB_MAXSIG} signals, so we allocate enough
@code{WFDB_Siginfo} objects to accommodate them all.
...
@i{10}      WFDB_Sample vin[WFDB_MAXSIG], vout[WFDB_MAXSIG];
@i{11}
@i{12}      if (argc < 2) @{
@i{13}          fprintf(stderr, "usage: %s record\n", argv[0]); exit(1);
@i{14}      @}
@i{15}      if ((nsig = isigopen(argv[1], s, WFDB_MAXSIG)) <= 0) exit(2);
...
@example
 ...
 @i{4}  int sample_ok = 1;
 @i{5}
 @i{6}  WFDB_Sample sample(s, t)
 @i{7}  WFDB_Signal s;
 @i{8}  WFDB_Time t;
 @i{9}  @{
@i{10}      static WFDB_Sample sbuf[BUFLN][WFDB_MAXSIG];
@i{11}      static WFDB_Time tt = -1L;
@i{12}
@i{13}      if (t <= tt - BUFLN)
...
@i{25}      int i, j, nc = argc - 4, nsig, v[WFDB_MAXSIG];
@i{26}      long nsamp, t;
@i{27}      static WFDB_Siginfo s[WFDB_MAXSIG];
@i{28}
@i{29}      if (argc < 4) @{
@i{30}          fprintf(stderr,
...
@i{45}      if ((nsig = isigopen(argv[1], s, WFDB_MAXSIG)) < 1)
@i{46}          exit(3);
...
@example
 ...
 @i{7}      int i, nsig = 0, v[WFDB_MAXSIG];
 @i{8}      long nsamp, t;
 @i{9}      double freq = 0.;
@i{10}      static char filename[WFDB_MAXSIG][32],
@i{11}          description[WFDB_MAXSIG][32], units[WFDB_MAXISIG][22];
@i{12}      static WFDB_Siginfo s[WFDB_MAXSIG];
@i{13}
@i{14}      do @{
...
@i{22}      @} while (nsig < 1 || nsig > WFDB_MAXSIG);
@i{23}      do @{
...
@i{10}      WFDB_Sample v[WFDB_MAXSIG], vb[WFDB_MAXSIG];
@i{11}      long stoptime = 0L, *sum[WFDB_MAXSIG];
@i{12}      WFDB_Anninfo a;
@i{13}      WFDB_Annotation annot;
@i{14}      static WFDB_Siginfo s[WFDB_MAXSIG];
@i{15}      void *calloc();
@i{16}
@i{17}      if (argc < 3) @{
@i{18}          fprintf(stderr,
...
@i{24}      if ((nsig = wfdbinit(argv[2], &a, 1, s, WFDB_MAXSIG)) < 1) exit(2);
...
@i{13}          ms160, ms200, s2, scmax, scmin = 0, v[WFDB_MAXSIG];
@i{14}      WFDB_Anninfo a;
@i{15}      WFDB_Annotation annot;
@i{16}      static WFDB_Siginfo s[WFDB_MAXSIG];
@i{17}  
@i{18}      if (argc < 2) @{
@i{19}          fprintf(stderr, "usage: %s record [threshold]\n", argv[0]);
@i{20}          exit(1);
@i{21}      @}
@i{22}      a.name = argv[0]; a.stat = WFDB_WRITE;
@i{23}      if (wfdbinit(argv[1], &a, 1, s, WFDB_MAXSIG) < 1) exit(2);
...
@code{WFDB_MAXANN} (defined in @file{<wfdb/wfdb.h>}) input or output
annotation files may be open at once, the largest valid annotator number
is @code{WFDB_MAXANN - 1}.
...
versions of the WFDB library can locate and read @file{header} files named
using either convention.
...
Package includes @code{wfdbcollate} (under MS-DOS, @code{wfdbcoll}), an
...
order beginning with 0.  Since at most @code{WFDB_MAXSIG} (defined in
@file{<wfdb/wfdb.h>}) input or output signals may be open at once, the
largest valid signal number is @code{WFDB_MAXSIG - 1}.
...
* Distribution::	How to obtain the latest version of the WFDB
			Software Package.
* UNIX installation::	Installing the WFDB Software Package on UNIX and
			similar systems.
...
diskettes.  For information on obtaining the latest version of the WFDB
Software Package, @pxref{Sources}.

The WFDB library is provided in precompiled form for MS-DOS users,
...
the WFDB applications included in the WFDB Software Package.  To use the
precompiled WFDB library, you must compile and link your own programs with
...
the WFDB Software Package, see the files @file{readme.doc} (in the root
directory of the CD-ROM) and @file{lib.doc} (in the @file{lib}
directory) for information about using the precompiled WFDB library, and
check @file{lib\ecg\wfdb.h} to see which WFDB library version is on your
CD-ROM.

On more recent (1995 and later) CD-ROMs, the WFDB Software Package is provided
in source form within @file{software/wfdb}, in precompiled binary form for
...
for several versions of UNIX.  The WFDB Browser for MS Windows
...
4.x, SPARC Solaris 2.x, and Linux).  If you are installing the WFDB
...
@file{src/wfdb} directory to your writable file system.  One way to do this is
...
If you have obtained the WFDB Software Package on a diskette, copy the
file @file{wfdb-tar.z} from the diskette to your writable file system.
(Note that @file{wfdb-tar.z} is a binary file;  do not remove carriage return
characters from it as you would normally do if copying an MS-DOS text file.)
Now type:

@example
uncompress <wfdb-tar.z | tar xfv -
@end example

@noindent
This command unpacks the archive into a directory called @file{wfdb}.  You may
remove @file{wfdb-tar.z} from your hard disk now if you wish.  (If you don't
...
once the archive has been unpacked.  Copy the entire contents of the @file{wfdb}
directory to your UNIX file system, treating all files as binary files.)

Change to the @file{wfdb} directory on your hard disk:
...
Follow the instructions in file @file{UNIX.TXT}, within the @file{wfdb}
...
the @file{src/wfdb} directory to your hard disk.  If you are installing
...
archive on a PC and copy the entire contents of the @file{wfdb} directory
...
in file @file{MAC} (within the @file{wfdb} directory).
...
(@pxref{WFDB path}).  Programs that accept @emph{time} arguments or commands
...
annotators, simply change @code{WFDB_MAXSIG} or @code{WFDB_MAXANN} in @file{wfdb.h},
and recompile the WFDB library and your applications.  Your operating system's
limit on the number of simultaneously open files may require you to use
multiplexed signal files if you increase @code{WFDB_MAXSIG} substantially.
...
installed the WFDB Software Package, you may find an updated version of
this list in @file{SOURCES.TXT}, in the @file{wfdb} directory that was
...
conference.  CIC will be in Hannover (Germany) in 1999.
...
Tenth Edition (revised for WFDB library version 10.0.0)

George B. Moody

Copyright (C) 1989 -- 1999 George B. Moody
