...
(@code{http://@-www@-.delorie@-.com/@-djgpp/}). These are used in exactly
the same way as described above for Unix C compilers..
...
in this guide; for further information, refer to @file{fortran/README}
...
WFDB applications make use of two @dfn{environment variables}, which are
named @code{WFDB} and @code{WFDBCAL}.  If these variables have not been
otherwise defined by the user, their values are those given by
@code{DEFWFDBP} and @code{DEFWFDBC} (defined in @file{wfdblib.h} at the
...
WFDB library refers to a character string (the value of the @code{WFDB}
environment variable) that consists of an ordered list of locations to
be searched @emph{for input files}.  This string is called the
@dfn{database path}, or the @dfn{WFDB path}.
...
such as @file{My Documents} to store WFDB files).}
...
file that resides in the first of these locations named in the WFDB
path.  Any other matching files are effectively invisible to WFDB
applications unless the WFDB path is rearranged.
...
@code{NETFILES} support is not compiled into the WFDB library, any WFDB
...
library if necessary in this case.)
...
Current versions of the WFDB library are compiled with a two-component
default WFDB path; the first component is empty (i.e., it refers
to the current directory), and the second component is
@code{http://www.physionet.org/physiobank/database} (referring to the
...
If you need to use a non-default WFDB path, you must set the @code{WFDB}
...
interpreters; see @emph{setdb}(1), in the @cite{WFDB Applications
Guide}.

@cindex calibration file
@cindex @code{WFDBCAL} (environment variable)
These shell scripts also set the @code{WFDBCAL} environment variable,
...
of the directories named by @code{WFDB}.  (The symbol @code{DEFWFDBC}
is usually defined in @file{wfdblib.h} to specify the name of a default
calibration file, to be used by the WFDB library if @code{WFDBCAL} has
not been set.)  Each signal type may be represented by an entry in the
...
respectively, so you will need to make a few changes to the program in
order to read these records successfully.
...
@noindent
This function creates a @file{hea} file (in the current directory).
...
file for record @file{old} is used to create a header file for
record @file{new}:
...
number of segments in @var{snarray}.  Once a header has been
...
Several special-purpose header files allow application programs
...
@dfn{Piped record} header files allow application programs to
...
@i{31}      if (argc < 4) @{
@i{32}          fprintf(stderr,
@i{33}            "usage: %s record start duration [ coefficients ... ]\n",
@i{34}                  argv[0]);
@i{35}          exit(1);
@i{36}      @}
@i{37}      if (nc < 1) @{
...
@i{42}          fprintf(stderr, "%s: too many coefficients\n", argv[0]);
@i{43}          exit(2);
@i{44}      @}
@i{45}      for (i = 0; i < nc; i++)
...
@i{61}          fprintf(stderr, "%s: inappropriate value for duration\n",
@i{62}                  argv[0]);
@i{63}          exit(5);
@i{64}      @}
...
@i{35}              fprintf(stderr, "%s: insufficient memory\n", argv[0]);
@i{36}              exit(2);
@i{37}          @}
...
@i{33}      if (sampfreq(NULL) != 250.)
@i{34}          fprintf(stderr,"warning: %s is designed for 250 Hz input\n",
@i{35}                  argv[0]);
