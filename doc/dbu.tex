...
@sp 4
@center Tenth Edition (revised and with additions for WFDB library version 10.1.4)
@center @today
@sp 5
...
@file{100s.normal} (under MS-DOS, @file{100s.nor}) in the current
directory.  Now type ``@code{rdann -r 100s -a atruth}'' and observe the output
for a few seconds, then try ``@code{rdann -r 100s -a normal}'' and notice the
difference.
...
* Distribution::	How to obtain the latest version of the WFDB
			Software Package.
* UNIX installation::	Installing the WFDB Software Package on UNIX, Linux,
			and similar systems.
...
@unnumberedsec UNIX, Linux, and similar operating systems
...
@code{libwww-config}, then @code{libwww} is installed already.)  You may
...
(If you have GNU @code{tar}, as on Linux, you can combine these into a
...
@code{10.1.1}).  Enter this directory.
...
annotators, simply change @code{WFDB_MAXSIG} or @code{WFDB_MAXANN} in
@file{wfdb.h}, and recompile the WFDB library and your applications.  Your
operating system's limit on the number of simultaneously open files may require
you to use multiplexed signal files if you increase @code{WFDB_MAXSIG}
substantially.
